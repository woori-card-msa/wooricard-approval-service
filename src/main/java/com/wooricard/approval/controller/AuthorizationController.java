...
public class AuthorizationController {
    ...
    @PostMapping("/cancel")
    public ResponseEntity<CancelResponse> cancelAuthorization(@RequestBody CancelRequest request) {
        log.info("승인 취소 요청 수신 - 거래 ID: {}", request.getTransactionId());

        CancelResponse response = authorizationService.cancel(request);

        if (response.isCancelled()) {
            log.info("승인 취소 성공 - 거래 ID: {}", request.getTransactionId());
        } else {
            log.warn("승인 취소 실패 - 거래 ID: {}, 응답 코드: {}", request.getTransactionId(), response.getResponseCode());
        }
