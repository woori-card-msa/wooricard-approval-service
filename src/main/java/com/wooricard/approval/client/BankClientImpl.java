...
import com.wooricard.approval.dto.BalanceRequest;
import com.wooricard.approval.dto.BalanceResponse;
import com.wooricard.approval.dto.DebitRequest;
import com.wooricard.approval.dto.DebitResponse;
import com.wooricard.approval.dto.RefundRequest;
...
public class BankClientImpl implements BankClient {
    ...
    private static final Duration REFUND_TIMEOUT = Duration.ofSeconds(30);    // 환불 타임아웃: 30초
    ...
    public BankClientImpl(@Value("${bank.service.url:http://localhost:8080}") String bankServiceUrl) {
        ...
        // 환불용 RestClient (타임아웃: 30초)
        JdkClientHttpRequestFactory refundRequestFactory = new JdkClientHttpRequestFactory();
        refundRequestFactory.setReadTimeout(REFUND_TIMEOUT);

        this.refundRestClient = RestClient.builder()
                .baseUrl(bankServiceUrl)
                .requestFactory(refundRequestFactory)
                .build();
    }
    ...
    @Override
    public RefundResponse requestRefund(String cardNumber, BigDecimal amount, String transactionId) {
        RefundRequest request = new RefundRequest(cardNumber, amount, transactionId);
        return executeWithRetry(() -> performRefund(request), "환불");
    }
     ...
     * 환불 실행 (타임아웃: 30초)
     */
    private RefundResponse performRefund(RefundRequest request) {
        logger.info("은행 환불 요청: cardNumber={}, amount={}, transactionId={}",
                maskCardNumber(request.getCardNumber()), request.getAmount(), request.getTransactionId());

        try {
            RefundResponse response = refundRestClient.post()
                    .uri("/api/account/refund")
                    .contentType(MediaType.APPLICATION_JSON)
                    .body(request)
                    .retrieve()
                    .body(RefundResponse.class);

            logger.info("은행 환불 성공: transactionId={}", response.getTransactionId());
            return response;

        } catch (Exception e) {
            logger.error("은행 환불 실패: {}", e.getMessage());
            throw new BankClientException("환불 실패", e);
        }
    }
